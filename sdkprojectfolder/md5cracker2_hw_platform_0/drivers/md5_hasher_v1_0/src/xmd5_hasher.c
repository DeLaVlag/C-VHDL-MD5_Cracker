...
void XMd5_hasher_Set_msgColl(XMd5_hasher *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XMd5_hasher_WriteReg(InstancePtr->Axilites_BaseAddress, XMD5_HASHER_AXILITES_ADDR_MSGCOLL_DATA, Data);
}

u32 XMd5_hasher_Get_msgColl(XMd5_hasher *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XMd5_hasher_ReadReg(InstancePtr->Axilites_BaseAddress, XMD5_HASHER_AXILITES_ADDR_MSGCOLL_DATA);
    return Data;
}
